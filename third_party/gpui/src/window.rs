...
impl Window {
    ...
    pub fn dispatch_event(&mut self, event: PlatformInput, cx: &mut App) -> DispatchEventResult {
        ...
        let event = match event {
            ...
            PlatformInput::Touch(touch_event) => {
                self.modifiers = touch_event.modifiers;
                PlatformInput::Touch(touch_event)
            }
            PlatformInput::MagnifyGesture(magnify_gesture) => {
                self.mouse_position = magnify_gesture.position;
                self.modifiers = magnify_gesture.modifiers;
                PlatformInput::MagnifyGesture(magnify_gesture)
            }
            PlatformInput::RotateGesture(rotate_gesture) => {
                self.mouse_position = rotate_gesture.position;
                self.modifiers = rotate_gesture.modifiers;
                PlatformInput::RotateGesture(rotate_gesture)
            }
            PlatformInput::SwipeGesture(swipe_gesture) => {
                self.mouse_position = swipe_gesture.position;
                self.modifiers = swipe_gesture.modifiers;
                PlatformInput::SwipeGesture(swipe_gesture)
            }
            PlatformInput::SmartMagnifyGesture(smart_magnify_gesture) => {
                self.mouse_position = smart_magnify_gesture.position;
                self.modifiers = smart_magnify_gesture.modifiers;
                PlatformInput::SmartMagnifyGesture(smart_magnify_gesture)
            }
