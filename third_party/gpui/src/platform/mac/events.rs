use crate::{
    ...
    MagnifyGestureEvent, MouseDownEvent, MouseExitEvent, MouseMoveEvent, MouseUpEvent,
    NavigationDirection, Pixels, PlatformInput, RotateGestureEvent, ScrollDelta,
    ScrollWheelEvent, SmartMagnifyGestureEvent, SwipeGestureEvent, TouchContact, TouchEvent,
    TouchPhase,
    platform::mac::{
...
const SMART_MAGNIFY_EVENT_TYPE: u64 = 32;
const TOUCH_PHASE_TOUCHING_MASK: NSUInteger = (1 << 0) | (1 << 1) | (1 << 2);
...
unsafe fn read_touch_phase(native_event: id) -> TouchPhase {
    unsafe {
...
unsafe fn read_position(native_event: id, window_height: Pixels) -> crate::Point<Pixels> {
    unsafe {
        point(
...
unsafe fn read_active_touches(native_event: id, view: id) -> SmallVec<[TouchContact; 8]> {
    unsafe {
        let touches: id =
            msg_send![native_event, touchesMatchingPhase: TOUCH_PHASE_TOUCHING_MASK inView: view];
        if touches.is_null() {
            return SmallVec::new();
        }

        let objects: id = msg_send![touches, allObjects];
        if objects.is_null() {
            return SmallVec::new();
        }

        let count: NSUInteger = msg_send![objects, count];
        let mut active_touches = SmallVec::new();

        for index in 0..count {
            let touch: id = msg_send![objects, objectAtIndex: index];
            if touch.is_null() {
                continue;
            }
            ...
            let is_resting: bool = {
                let resting: objc::runtime::BOOL = msg_send![touch, isResting];
                resting == YES
            };

            active_touches.push(TouchContact {
        ...
        active_touches.sort_unstable_by_key(|touch: &TouchContact| touch.id);
        active_touches
    }
}
...
impl PlatformInput {
    pub(crate) unsafe fn from_native(
        ...
        view: Option<id>,
        touch_phase_override: Option<TouchPhase>,
    ) -> Option<Self> {
        unsafe {
            ...
            if let Some(touch_phase) = touch_phase_override {
                let view = view?;
                return Some(Self::Touch(TouchEvent {
                    touches: read_active_touches(native_event, view),
            ...
            match event_type {
                ...
                NSEventType::NSEventTypeMagnify => window_height.map(|window_height| {
                    Self::MagnifyGesture(MagnifyGestureEvent {
                        position: read_position(native_event, window_height),
                        magnification: native_event.magnification() as f32,
                        modifiers: read_modifiers(native_event),
                        touch_phase: read_touch_phase(native_event),
                    })
                }),
                NSEventType::NSEventTypeRotate => window_height.map(|window_height| {
                    Self::RotateGesture(RotateGestureEvent {
                        position: read_position(native_event, window_height),
                        rotation_degrees: native_event.rotation(),
                        modifiers: read_modifiers(native_event),
                        touch_phase: read_touch_phase(native_event),
                    })
                }),
                NSEventType::NSEventTypeSwipe => window_height.map(|window_height| {
                    Self::SwipeGesture(SwipeGestureEvent {
                        position: read_position(native_event, window_height),
                        delta: point(native_event.deltaX() as f32, native_event.deltaY() as f32),
                        modifiers: read_modifiers(native_event),
                        touch_phase: read_touch_phase(native_event),
                    })
                }),
                event_type if event_type as u64 == SMART_MAGNIFY_EVENT_TYPE => {
                    window_height.map(|window_height| {
                    Self::SmartMagnifyGesture(SmartMagnifyGestureEvent {
                        position: read_position(native_event, window_height),
                        modifiers: read_modifiers(native_event),
                    })
                    })
