...
use crate::{
    ...
    SharedString, Size, SystemWindowTab, Timer, TouchPhase, WindowAppearance,
        ...
        VIEW_CLASS = {
            ...
            unsafe {
                ...
                decl.add_method(
                    ...
                    handle_touch_began as extern "C" fn(&Object, Sel, id),
                );
                decl.add_method(
                    sel!(touchesMovedWithEvent:),
                    handle_touch_moved as extern "C" fn(&Object, Sel, id),
                );
                decl.add_method(
                    sel!(touchesEndedWithEvent:),
                    handle_touch_ended as extern "C" fn(&Object, Sel, id),
                );
                decl.add_method(
                    sel!(touchesCancelledWithEvent:),
                    handle_touch_cancelled as extern "C" fn(&Object, Sel, id),
                );
                decl.add_method(
                    sel!(flagsChanged:),
                    handle_view_event as extern "C" fn(&Object, Sel, id),
...
impl MacWindow {
    pub fn open(
        ...
        unsafe {
            ...
            let _: () = msg_send![native_view, setAcceptsTouchEvents: YES];
            let _: () = msg_send![native_view, setAllowedTouchTypes: 1usize << 1];
...
extern "C" fn handle_key_event(this: &Object, native_event: id, key_equivalent: bool) -> BOOL {
    ...
    let event = unsafe { PlatformInput::from_native(native_event, Some(window_height), None, None) };
...
extern "C" fn handle_view_event(this: &Object, _: Sel, native_event: id) {
    ...
    let event = unsafe {
        PlatformInput::from_native(
            native_event,
            Some(window_height),
            Some(this as *const Object as *mut Object),
            None,
        )
    };
...
extern "C" fn handle_touch_began(this: &Object, _: Sel, native_event: id) {
    handle_touch_event(this, native_event, TouchPhase::Started);
}

extern "C" fn handle_touch_moved(this: &Object, _: Sel, native_event: id) {
    handle_touch_event(this, native_event, TouchPhase::Moved);
}

extern "C" fn handle_touch_ended(this: &Object, _: Sel, native_event: id) {
    handle_touch_event(this, native_event, TouchPhase::Ended);
}

extern "C" fn handle_touch_cancelled(this: &Object, _: Sel, native_event: id) {
    handle_touch_event(this, native_event, TouchPhase::Ended);
}

fn handle_touch_event(this: &Object, native_event: id, touch_phase: TouchPhase) {
    let window_state = unsafe { get_window_state(this) };
    let mut lock = window_state.as_ref().lock();
    let window_height = lock.content_size().height;
    let event = unsafe {
    ...
    if let Some(event) = event
        && let Some(mut callback) = lock.event_callback.take()
    {
        drop(lock);
        callback(event);
        window_state.lock().event_callback = Some(callback);
