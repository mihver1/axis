...
/// A raw touch contact tracked by the platform.
#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct TouchContact {
    /// A platform-specific stable identifier for the life of this touch.
    pub id: u64,

    /// The absolute normalized touch position on the device surface.
    ///
    /// Values are typically in `[0, 1]`, where `(0, 0)` is the lower-left corner of the touch
    /// surface.
    pub normalized_position: Point<f32>,

    /// Whether the touch is currently resting according to the platform.
    pub is_resting: bool,
}
...
pub struct TouchEvent {
    /// The currently active touches for this view.
    pub touches: SmallVec<[TouchContact; 8]>,

    /// The modifiers that were held down when the touch event was recognized.
    pub modifiers: Modifiers,
...
impl TouchEvent {
    /// Returns the centroid of the currently active touches in normalized touch-surface
    /// coordinates.
    pub fn centroid(&self) -> Option<Point<f32>> {
        ...
        let sum = self
            .touches
            .iter()
            .fold(point(0.0, 0.0), |acc, touch| {
                point(
                    acc.x + touch.normalized_position.x,
                    acc.y + touch.normalized_position.y,
                )
            });
        let count = self.touches.len() as f32;
        Some(point(sum.x / count, sum.y / count))
    }
}
...
/// A pinch magnification gesture from the platform, such as a trackpad pinch on macOS.
#[derive(Clone, Debug, Default)]
pub struct MagnifyGestureEvent {
    /// The position of the pointer on the window.
    pub position: Point<Pixels>,

    /// The delta magnification for this gesture sample.
    ///
    /// A value of `0.1` means "zoom in by 10%" and `-0.1` means "zoom out by 10%".
    pub magnification: f32,

    /// The modifiers that were held down when the gesture was recognized.
    pub modifiers: Modifiers,
...
impl Sealed for MagnifyGestureEvent {}
impl InputEvent for MagnifyGestureEvent {
    fn to_platform_input(self) -> PlatformInput {
        PlatformInput::MagnifyGesture(self)
    }
}
impl MouseEvent for MagnifyGestureEvent {}

impl Deref for MagnifyGestureEvent {
    type Target = Modifiers;
...
/// A rotation gesture from the platform, such as a two-finger rotate on macOS.
#[derive(Clone, Debug, Default)]
pub struct RotateGestureEvent {
    /// The position of the pointer on the window.
    pub position: Point<Pixels>,

    /// The rotation delta in degrees for this gesture sample.
...
impl Sealed for RotateGestureEvent {}
impl InputEvent for RotateGestureEvent {
    fn to_platform_input(self) -> PlatformInput {
        PlatformInput::RotateGesture(self)
    }
}
impl MouseEvent for RotateGestureEvent {}

impl Deref for RotateGestureEvent {
    type Target = Modifiers;
...
/// A swipe gesture from the platform, such as a multi-finger swipe on macOS.
#[derive(Clone, Debug, Default)]
pub struct SwipeGestureEvent {
    /// The position of the pointer on the window.
    pub position: Point<Pixels>,

    /// The gesture delta. On macOS, swipe events are typically normalized to `-1`, `0`, or `1`
    /// per axis rather than raw pixels.
    pub delta: Point<f32>,
...
impl Sealed for SwipeGestureEvent {}
impl InputEvent for SwipeGestureEvent {
    fn to_platform_input(self) -> PlatformInput {
        PlatformInput::SwipeGesture(self)
    }
}
impl MouseEvent for SwipeGestureEvent {}

impl Deref for SwipeGestureEvent {
    type Target = Modifiers;
...
/// A smart magnify gesture from the platform, such as a two-finger double tap on macOS.
#[derive(Clone, Debug, Default)]
pub struct SmartMagnifyGestureEvent {
    /// The position of the pointer on the window.
    pub position: Point<Pixels>,
...
impl Sealed for SmartMagnifyGestureEvent {}
impl InputEvent for SmartMagnifyGestureEvent {
    fn to_platform_input(self) -> PlatformInput {
        PlatformInput::SmartMagnifyGesture(self)
    }
}
impl MouseEvent for SmartMagnifyGestureEvent {}

impl Deref for SmartMagnifyGestureEvent {
    type Target = Modifiers;
...
pub enum PlatformInput {
    ...
    /// A raw touch event was recognized.
    Touch(TouchEvent),
    /// A magnify gesture was recognized.
    MagnifyGesture(MagnifyGestureEvent),
    /// A rotation gesture was recognized.
    RotateGesture(RotateGestureEvent),
    /// A swipe gesture was recognized.
    SwipeGesture(SwipeGestureEvent),
    /// A smart magnify gesture was recognized.
    SmartMagnifyGesture(SmartMagnifyGestureEvent),
...
impl PlatformInput {
    pub(crate) fn mouse_event(&self) -> Option<&dyn Any> {
        match self {
            ...
            PlatformInput::MagnifyGesture(event) => Some(event),
            PlatformInput::RotateGesture(event) => Some(event),
            PlatformInput::SwipeGesture(event) => Some(event),
            PlatformInput::SmartMagnifyGesture(event) => Some(event),
    ...
    pub(crate) fn keyboard_event(&self) -> Option<&dyn Any> {
        match self {
            ...
            PlatformInput::MagnifyGesture(_) => None,
            PlatformInput::RotateGesture(_) => None,
            PlatformInput::SwipeGesture(_) => None,
            PlatformInput::SmartMagnifyGesture(_) => None,
